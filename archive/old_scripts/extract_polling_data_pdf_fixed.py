    ...
    def extract_from_pdf(self, pdf_path: str, max_retries: int = 2) -> List[Dict]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = self.client.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        ...
                        thinking_config=types.ThinkingConfig(
                            thinking_budget=1024  # Use thinking for better reasoning
                        )
                    )
                )
                ...
                if not response:
                    print(f"⚠ No response received for {Path(pdf_path).name}")
                ...
                if hasattr(response, 'prompt_feedback') and response.prompt_feedback:
                    if hasattr(response.prompt_feedback, 'block_reason') and response.prompt_feedback.block_reason:
                        print(f"✗ Request was blocked for {Path(pdf_path).name}. Reason: {response.prompt_feedback.block_reason}")
                        return []
                ...
                try:
                    ...
                    if not response_text or response_text.strip() == "":
                        print(f"⚠ Empty response for {Path(pdf_path).name}")
                        if attempt == max_retries - 1:
                            return []
                        time.sleep(2)
                        continue
                except (ValueError, AttributeError) as e:
                    print(f"⚠ Error accessing response text for {Path(pdf_path).name}: {e}")
                    if attempt == max_retries - 1:
                        return []
                    time.sleep(2)
                    continue
                ...
                if not isinstance(extracted_data, list):
                    print(f"⚠ Response is not a JSON array for {Path(pdf_path).name}")
                    if attempt == max_retries - 1:
                        return []
                    time.sleep(2)
                    continue
                ...
                print(f"⚠ Error processing {Path(pdf_path).name} (attempt {attempt + 1}): {e}")
                if attempt == max_retries - 1:
    ...
    def process_directory(self, pdf_dir: str = "polling_pdfs") -> pd.DataFrame:
        ...
        for pdf_file in pdf_files:
            try:
                ...
                if processed_count % 3 == 0:  # Reduced frequency
                    print(f"Processed {processed_count}/{len(pdf_files)} files... pausing briefly")
                    time.sleep(3)  # Slightly longer pause
...
def main():
    ...
    try:
        ...
        if not df.empty:
            ...
            print(f"\n=== RECORDS BY CATEGORY ===")
            if 'Category' in df.columns and 'Survey_Organisation' in df.columns:
            ...
            if 'Category' in df.columns:
                ...
                if len(ai_reg) > 0 and 'Survey_Organisation' in ai_reg.columns:
                    print(f"Organizations with AI_Regulation questions: {', '.join(ai_reg['Survey_Organisation'].unique())}")
                if len(ai_reg) > 0 and 'Country' in ai_reg.columns:
