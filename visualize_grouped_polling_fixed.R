# AI Polling Data Visualization - Grouped Questions (Fixed Version)
# Improved script with better data handling and error prevention
...
# Load the full dataset with all polling information
full_data <- read_csv("extracted_data/polling_data_latest.csv", 
                     show_col_types = FALSE)

# Load the grouped data with question group assignments
...
# Clean and merge datasets properly
...
# Merge datasets with proper relationship handling
grouped_data <- full_data %>%
  inner_join(
    group_assignments_clean,
    by = "question_text",
    relationship = "many-to-one"  # Specify the expected relationship
  ) %>%
  # Clean data
  mutate(
    ...
    country_clean = case_when(
      str_detect(country, "United States|USA|US") ~ "United States",
  ...
  filter(
    ...
    agreement >= 0,
    agreement <= 100,
    !str_detect(question_group, "^ungrouped_")  # Remove ungrouped questions
  )

print(sprintf("✅ Successfully processed data: %d questions in %d groups", 
              nrow(grouped_data), 
...
group_stats <- grouped_data %>%
  ...
  summarise(
    ...
    date_span = as.numeric(max(fieldwork_date, na.rm = TRUE) - min(fieldwork_date, na.rm = TRUE)),
    mean_agreement = mean(agreement, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  # Focus on groups with meaningful data
  filter(
    n_questions >= 3,           # At least 3 questions
    n_timepoints >= 2,          # At least 2 time points
    date_span >= 30             # At least 30 days span
  ) %>%
  arrange(desc(n_questions))

print(sprintf("📈 Found %d groups with sufficient data for visualization", nrow(group_stats)))
...
# Improved function to create grouped visualization
create_group_visualization <- function(group_name, data) {
  ...
  # Calculate weighted average for the entire group
  group_weighted_avg <- group_data %>%
    filter(!is.na(agreement), !is.na(n_respondents), n_respondents > 0) %>%
    summarise(
      weighted_agreement = sum(agreement * n_respondents, na.rm = TRUE) / sum(n_respondents, na.rm = TRUE),
      total_sample_size = sum(n_respondents, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    pull(weighted_agreement)
  
  # Create time series data with proper aggregation
  time_series_data <- group_data %>%
    # Group by country and date to handle multiple questions per survey
    ...
    summarise(
      ...
      organizations = str_trunc(paste(unique(survey_organisation), collapse = "; "), 100),
      # Get all question texts for this group
      all_questions = paste(unique(str_trim(question_text)), collapse = " | "),
      sample_question = str_trunc(first(question_text), 150),
      total_sample_size = sum(n_respondents, na.rm = TRUE),
      mean_sample_size = mean(n_respondents, na.rm = TRUE),
      .groups = "drop"
    ) %>%
  ...
  if (nrow(time_series_data) < 2) return(NULL)
  
  # Create the plot with source for click events
  p <- plot_ly(source = "scatter_plot") %>%
    layout(
      title = list(
        text = paste0("<b>", str_replace_all(group_name, "_", " "), "</b><br>",
                     "<sub>Public Opinion Trends Over Time (", nrow(time_series_data), " data points) • Dot size = Sample size</sub>"),
        x = 0.5,
        font = list(size = 16)
      ),
      xaxis = list(
  ...
  for (country in unique(time_series_data$country_clean)) {
    country_data <- time_series_data %>% 
      filter(country_clean == country) %>%
      arrange(fieldwork_date)
    
    if (nrow(country_data) >= 1) {
      # Calculate marker sizes based on sample size (scale between 6 and 20)
      ...
      p <- p %>%
        add_trace(
          ...
          text = paste0(
            ...
            "Orgs: ", str_trunc(country_data$organizations, 50), "<br>",
            "Sample: ", format(country_data$total_sample_size, big.mark = ","), "<br><br>",
            "<b>Survey Questions:</b><br>",
            ifelse(is.na(country_data$all_questions) | country_data$all_questions == "", 
                   "No question text available", 
                   str_wrap(str_trunc(country_data$all_questions, 600), width = 80))
          )
        )
  ...
  # Add weighted average horizontal line
  if (!is.na(group_weighted_avg) && group_weighted_avg > 0) {
    date_range <- range(time_series_data$fieldwork_date, na.rm = TRUE)
    p <- p %>%
      add_trace(
        x = date_range,
        ...
        hovertemplate = paste0(
          ...
          "Based on all studies in this group<br>",
          "<extra></extra>"
        )
      )
  }
  ...
  sample_q <- str_trunc(time_series_data$sample_question[1], 200)
  p <- p %>%
    layout(
      annotations = list(
        list(
          text = paste0("Sample question: ", sample_q),
          showarrow = FALSE,
          x = 0,
          y = -0.2,
          xref = "paper",
...
# Create visualizations for groups with sufficient data
print("🎨 Creating visualizations for question groups...")

# Process all groups with sufficient data
